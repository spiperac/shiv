...
func makeWSTab(name, url string, tls bool) store.RepeaterTab {
	return store.RepeaterTab{
		Name:       name,
		Host:       url,
		Port:       0,
...
func TestSaveRepeaterTab_PersistsAllFields(t *testing.T) {
	...
	assert.Equal(t, "http", got.TabType)
...
func TestSaveRepeaterTab_TLSFalse(t *testing.T) {
	...
	assert.Equal(t, "http", tabs[0].TabType)
}

func TestSaveRepeaterTab_DefaultTabType(t *testing.T) {
	st := newTestStore(t)

	// TabType omitted — should default to "http".
	tab := store.RepeaterTab{
		Name:       "No Type",
		Host:       "example.com",
		Port:       80,
...
func TestSaveRepeaterTab_WebSocketType(t *testing.T) {
	st := newTestStore(t)

	tab := makeWSTab("WS Tab", "wss://echo.websocket.org:443/", true)
	id, err := st.SaveRepeaterTab(tab)
	require.NoError(t, err)
	assert.Positive(t, id)
	...
	require.NoError(t, err)
	require.Len(t, tabs, 1)
	assert.Equal(t, "websocket", tabs[0].TabType)
	assert.Equal(t, "wss://echo.websocket.org:443/", tabs[0].RawRequest)
	assert.True(t, tabs[0].TLS)
}

func TestAllRepeaterTabs_MixedTypes(t *testing.T) {
	st := newTestStore(t)

	_, err := st.SaveRepeaterTab(makeTab("HTTP", "example.com", 80, false))
	require.NoError(t, err)
	_, err = st.SaveRepeaterTab(makeWSTab("WS", "wss://example.com/ws", true))
	require.NoError(t, err)
	_, err = st.SaveRepeaterTab(makeTab("HTTP2", "other.com", 443, true))
	require.NoError(t, err)
	...
	require.Len(t, tabs, 3)
	assert.Equal(t, "http", tabs[0].TabType)
	assert.Equal(t, "websocket", tabs[1].TabType)
	assert.Equal(t, "http", tabs[2].TabType)
}

func TestSaveRepeaterTab_WebSocketPreservesURL(t *testing.T) {
	st := newTestStore(t)

	url := "wss://api.example.com:8443/realtime?token=abc123"
	tab := makeWSTab("API WS", url, true)
	id, err := st.SaveRepeaterTab(tab)
	require.NoError(t, err)

	full, err := st.AllRepeaterTabs()
	require.NoError(t, err)
	require.Len(t, full, 1)
	assert.Equal(t, id, full[0].ID)
	assert.Equal(t, url, full[0].RawRequest)
	assert.Equal(t, "websocket", full[0].TabType)
}

func TestDeleteRepeaterTab_WebSocket(t *testing.T) {
	st := newTestStore(t)

	httpID, err := st.SaveRepeaterTab(makeTab("HTTP", "example.com", 80, false))
	require.NoError(t, err)
	wsID, err := st.SaveRepeaterTab(makeWSTab("WS", "wss://example.com/ws", true))
	require.NoError(t, err)

	require.NoError(t, st.DeleteRepeaterTab(wsID))

	tabs, err := st.AllRepeaterTabs()
	require.NoError(t, err)
	require.Len(t, tabs, 1)
	assert.Equal(t, httpID, tabs[0].ID)
	assert.Equal(t, "http", tabs[0].TabType)
}
